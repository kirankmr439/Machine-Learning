function [w, iterations,error,od,count,eta1,time_elapsed]=DeltaTraining_second_deacy(Data, Target, eta, epochs)
[rd, cd]=size(Data);
[rt, ct]=size(Target);
if rt ~= rt
...
while  iterations <= epochs 
   ...
   out=[];
  eta(iterations)=power(eta1(iterations),iterations);
 for i=1:rd-1,
 temp=sum(w .* [1,Data(i,:)]);
    if temp < 0
         out(i) = -1;
